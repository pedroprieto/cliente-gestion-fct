<template>
  <div class="box">
    <div class="level is-size-4">
      ...
      <div class="level-item has-text-centered">
        Importe total:
        <span class="has-text-weight-bold">{{ FCT.kmsvisitas.importe.toFixed(2) }} €</span>
      </div>
    </div>
  </div>
  <div class="level">
    ...
    <div class="level-right">
      <div class="level-item">
        <button
          ...
          @click="FCT.generarCertificados('cert_alumno')"
          class="button is-outlined is-primary"
        >
          <span class="icon">
            <svg-icon class="mdi" type="mdi" :path="mdiAccountSchool"></svg-icon>
          </span>
          <span> Certificados de alumado </span>
        </button>
      </div>
      <div class="level-item">
        <button
        ...
        >
          ...
          <span> Certificados de instructores </span>
        </button>
      </div>
      <div class="level-item">
        <button
          :disabled="!anySelected"
          @click="FCT.generarCertificados('fm18')"
          class="button is-outlined is-primary"
        >
          <span class="icon">
  ...
  <div class="box cursorPointer" @click="toggleSelected()" v-if="this.FCT.fcts.length > 1">
    <div class="columns">
      ...
      <div class="column">Seleccionar todas/ninguna</div>
    </div>
  </div>

  <div
    @click="selectRow(fct)"
    class="box cursorPointer"
    :class="{ 'is-selected': fct.selected }"
    v-for="fct of filteredFCTs"
    :key="fct.id"
  >
    <div class="columns is-multiline is-align-items-center">
      ...
      <div class="columns column is-multiline">
        ...
        <div class="column is-2">
          <abbr title="Fecha de inicio">F. ini.</abbr>
          <b>{{ new Date(fct.fecha_inicio).toLocaleDateString() }}</b>
        </div>
        <div class="column is-2">
          <abbr title="Fecha de finalización">F. fin.</abbr>
          <b>{{ new Date(fct.fecha_fin).toLocaleDateString() }}</b>
        </div>
        <div class="column is-2">
        ...
        <div class="column is-3">
          <span class="tags">
            <span
              class="tag"
              :class="{ 'is-success': fct.visita_ini, 'is-warning': !fct.visita_ini }"
              >Inicial</span
            >
            <span
              class="tag"
              :class="{ 'is-success': fct.visita_seg, 'is-warning': !fct.visita_seg }"
              >Seguimiento</span
            >
            <span
              class="tag"
              :class="{ 'is-success': fct.visita_fin, 'is-warning': !fct.visita_fin }"
              >Final</span
            >
          </span>
        </div>
        ...
        <div class="column is-5">
          <div class="buttons is-justify-content-flex-end">
            ...
            <button
              title="Certificado de alumno"
              class="button is-info"
              @click.stop="FCT.generarCertificados('cert_alumno', fct)"
            >
              <span class="icon">
                <svg-icon class="mdi" type="mdi" :path="mdiAccountSchool"></svg-icon>
              </span>
            ...
            <button
              ...
              class="button is-info"
              @click.stop="FCT.generarCertificados('cert_instructor', fct)"
            >
              <span class="icon">
                <svg-icon class="mdi" type="mdi" :path="mdiHumanMaleBoard"></svg-icon>
              </span>
            ...
            <button
              title="FM18"
              class="button is-info"
              @click.stop="FCT.generarCertificados('fm18', fct)"
            >
              <span class="icon">
                <svg-icon class="mdi" type="mdi" :path="mdiClipboardListOutline"></svg-icon>
              </span>
