<template>
<div class="panel" v-for="fct of fcts" :key="fct.id">
  ...
  <p class="panel-tabs">
    ...
    <div v-if="!fct.selectedTab">
      <p class="panel-block">
        Inicio: {{ fct.fecha_inicio.toLocaleDateString('es') }} - Fin:
        {{ fct.fecha_fin.toLocaleDateString('es') }}
      </p>
      <p class="panel-block">
        {{ fct.instructor }}
      </p>
...
export default {
  ...
  methods: {
    ...
    cancelarFormulario: function (fct) {
      ...
      const FCT = gestionFCTStore()
          if (!editing) {
              await FCT.addVisit(visitData, fct)
      } else {
      ...
      const FCT = gestionFCTStore()
      await FCT.deleteVisit(fct, visita)
          }
    },
    generarCertificado: function (tipo, fct) {
      const FCT = gestionFCTStore()
      FCT.generarCertificados(tipo, fct)
    }
  }
}
</script>
