<script setup>
  ...
  const rutas = router.getRoutes().filter(route => (route.name != 'login') && (route.name!='anyadirVisita') && (route.name != 'editarVisita') && (route.name !='visitas') && (route.name !='etiquetas') && (route.name!='root'));
    ...
    <nav class="navbar is-primary has-shadow" role="navigation" aria-label="main navigation">
      <div class="navbar-brand">
        <!-- <a class="navbar-item"> -->
          <!--   GestiónFCT -->
          <!-- </a> -->
        <a role="button" :class="{'is-active': state.navbar_activa}" class="navbar-burger" aria-label="menu" aria-expanded="false" data-target="navbar-app" @click="toggleNavbar()">
      ...
      <div :class="{'is-active': state.navbar_activa}" class="navbar-menu" id="navbar-app">
        <div class="navbar-start">
          <RouterLink v-for="ruta of rutas" :key="ruta.id" :class="{'is-active': rutaActual.name == ruta.name}" class="navbar-item" :to="{name: ruta.name, params: {curso: FCT.curso, periodo: FCT.periodo}}">{{ruta.meta.prompt}}</RouterLink>
        </div>
